library(dplyr)

### read data from file
train <- read.table(".//UCI HAR Dataset//train//X_train.txt", header = FALSE, 
                    sep = "", quote = "\"'")
train_label <- read.table(".//UCI HAR Dataset//train//y_train.txt", header = FALSE, 
                        sep = "", quote = "\"'")
train_subject <- read.table(".//UCI HAR Dataset//train//subject_train.txt", header = FALSE, 
                          sep = "", quote = "\"'")

test <- read.table(".//UCI HAR Dataset//test//X_test.txt", header = FALSE, 
                 sep = "", quote = "\"'")
test_label <- read.table(".//UCI HAR Dataset//test//y_test.txt", header = FALSE, 
                       sep = "", quote = "\"'")
test_subject <- read.table(".//UCI HAR Dataset//test//subject_test.txt", header = FALSE, 
                         sep = "", quote = "\"'")
## links the class labels with their activity name
activity_labels <- read.table(".//UCI HAR Dataset//activity_labels.txt", header = FALSE, 
                                          sep = "", quote = "\"'")
## rename features activity_labels
colnames(activity_labels) <- make.names(c("label", "activity"))


### construct complete train and test data
train$label <- train_label[,1]
train$subject <- train_subject[,1]

test$label <- test_label[,1]
test$subject <- test_subject[,1]

### merge train and test data
complete_data <- rbind(train, test)

### change type of features

### extracts features which I am interested
mean_std_indexes <- c(1,2,3,4,5,6, 41,42,43,44,45,46, 81,82,83,84,85,86, 121,122,123,124,125,126,
                      161,162,163,164,165,166, 201,202, 214,215, 227,228, 240,241, 253,254, 
                      266,267,268,269,270,271, 345,346,347,348,349,350, 424,425,426,427,428,429, 
                      503,504, 516,517, 529,530, 542,543)
mean_std_data <- complete_data[,mean_std_indexes]
mean_std_data$label <- complete_data$label
mean_std_data$subject <- complete_data$subject

### convert label to actual activity name
mean_std_data <- merge(mean_std_data, activity_labels, by = "label", all.x = T)
mean_std_data$label <- NULL

### make names for each feature
feature_names <- c("tBodyAcc-mean-X", "tBodyAcc-mean-Y", "tBodyAcc-mean-Z", "tBodyAcc-std-X", 
                    "tBodyAcc-std-Y", "tBodyAcc-std-Z", "tGravityAcc-mean-X", "tGravityAcc-mean-Y", 
                    "tGravityAcc-mean-Z", "tGravityAcc-std-X", "tGravityAcc-std-Y", "tGravityAcc-std-Z",
                    "tBodyAccJerk-mean-X", "tBodyAccJerk-mean-Y", "tBodyAccJerk-mean-Z", 
                    "tBodyAccJerk-std-X", "tBodyAccJerk-std-Y", "tBodyAccJerk-std-Z", 
                    "tBodyGyro-mean-X", "tBodyGyro-mean-Y", "tBodyGyro-mean-Z", "tBodyGyro-std-X", 
                    "tBodyGyro-std-Y", "tBodyGyro-std-Z", "tBodyGyroJerk-mean-X", "tBodyGyroJerk-mean-Y",
                    "tBodyGyroJerk-mean-Z", "tBodyGyroJerk-std-X", "tBodyGyroJerk-std-Y", 
                    "tBodyGyroJerk-std-Z", "tBodyAccMag-mean", "tBodyAccMag-std", "tGravityAccMag-mean",
                    "tGravityAccMag-std", "tBodyAccJerkMag-mean", "tBodyAccJerkMag-std", "tBodyGyroMag-mean",
                    "tBodyGyroMag-std", "tBodyGyroJerkMag-mean", "tBodyGyroJerkMag-std", "fBodyAcc-mean-X",
                    "fBodyAcc-mean-Y", "fBodyAcc-mean-Z", "fBodyAcc-std-X", "fBodyAcc-std-Y", 
                    "fBodyAcc-std-Z", "fBodyAccJerk-mean-X", "fBodyAccJerk-mean-Y", "fBodyAccJerk-mean-Z",
                    "fBodyAccJerk-std-X", "fBodyAccJerk-std-Y", "fBodyAccJerk-std-Z", "fBodyGyro-mean-X", 
                    "fBodyGyro-mean-Y", "fBodyGyro-mean-Z", "fBodyGyro-std-X", "fBodyGyro-std-Y", "fBodyGyro-std-Z",
                    "fBodyAccMag-mean", "fBodyAccMag-std", "fBodyBodyAccJerkMag-mean", "fBodyBodyAccJerkMag-std", 
                    "fBodyBodyGyroMag-mean", "fBodyBodyGyroMag-std", "fBodyBodyGyroJerkMag-mean", 
                    "fBodyBodyGyroJerkMag-std", "subject", "activity")
names(mean_std_data) <- make.names(feature_names)

### creates a second, independent tidy data set 
### with the average of each variable for each activity and each subject.
tidy_data <- mean_std_data %>%
    group_by(subject, activity) %>%
    summarise_each(funs(mean))

